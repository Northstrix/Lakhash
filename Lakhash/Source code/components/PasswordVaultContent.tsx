...
const extractRecordsFromFirebase = () => {
  ...
  const fetchLogins = async () => {
    ...
    try {
      ...
      const loginsRef = collection(db, `${user.email}/logins/enc`);
      ...
      for (const doc of querySnapshot.docs) {
        ...
        try {
          const new_iterations: number = Math.round(49 + (iterations / 37));
...
function pkcs7PaddingConsumed(data: Uint8Array) {
  ...
  const addRecord = async (newLogin: Omit<DecryptedLogin, 'id' | 'integrity'>) => {
    ...
    try {
      ...
      const docRef = doc(collection(db, `${user.email}/logins/enc`));
      ...
      toast.error(`Error adding login: ${error instanceof Error ? error.message : 'Unknown error'}`);
    } finally {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      // Create a document reference in Firestore
      const docRef = doc(db, `${user.email}/logins/enc`, currentRecord.id);
  ...
  return (
    <div className="encrypted-space">
      <main className="encrypted-space-main">
        <div className="encrypted-space-paper">
          ...
          <div className={`encrypted-space-lines ${isScrollable ? 'scrollable' : ''}`} ref={linesRef}>
            ...
            <div className="encrypted-space-record" onClick={toggleTheme}>
              Change Theme
            </div>
          </div>
              ...
              <div className="encrypted-space-pop-up-form">
                <div 
                  ...
                  style={{
                    position: 'absolute',
                    left: '50%',
                    transform: isMobile ? 'translateX(-50%)' : 'translateX(calc(-50% + 30px))',
                  }}
                >
      ...
      {/* Toast Notifications */}
      <ToastContainer position='bottom-right' style={{ bottom:'50px' }} />  
    </div>  
  );
}
